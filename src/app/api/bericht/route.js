...
import { createAndSend } from '@/lib/bericht';
export const dynamic = 'force-dynamic';
export const maxDuration = 300;

/** Bericht einer abgeschlossenen Runde erstellen und versenden: POST /api/bericht?round_id=N[&force=1][&resend=1] (Admin oder ?key=CRON_SECRET). */
export async function POST(req) {
  ...
  try { const log = await createAndSend(roundId, { force: !!sp.get('force'), resend: !!sp.get('resend') }); return NextResponse.json({ ok: true, log }); }

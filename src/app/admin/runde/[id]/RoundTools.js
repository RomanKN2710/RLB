...
export function Header({ round, managers, corrections }) {
  ...
  return (<div className="stack" style={{ marginTop: 8 }}>
    ...
    <div className="row"><label className="mini" style={{ flex: 1 }}>Team der Runde (Namen mit Komma; bei gleichen Nachnamen Verein in Klammern, z. B. «Schlotterbeck (Dortmund)») <input style={{ width: '100%' }} value={tdr} onChange={e => setTdr(e.target.value)} /></label><label className="mini">Spieler des Tages <input value={sdt} onChange={e => setSdt(e.target.value)} /></label><button className="sm komm" disabled={pending} onClick={() => run(() => roundInfoAction(round.id, tdr.split(',').map(s => s.trim()).filter(Boolean), sdt, label))}>Speichern</button></div>

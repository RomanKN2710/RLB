...
export default async function HallOfFame() {
  ...
  return (<>
    ...
    {H.count > 0 && <div className="card"><div className="eyebrow">Ewige Tabelle</div><h3>Alle Manager, sortiert nach Titeln, Podestplätzen und ewigen Punkten</h3>
      <div className="tbl"><table><thead><tr><th>#</th><th className="l">Manager</th><th className="l">Titel</th><th>🥈</th><th>🥉</th><th>Podest</th><th>Saisons</th><th>Ø Platz</th><th>Bester</th><th>Punkte</th><th className="l">Dabei</th></tr></thead>
        <tbody>{H.table.map((x, i) => <tr key={x.manager} className={`${x.manager === me ? 'me' : ''} ${active.has(x.manager) ? '' : 'off'}`}><td>{i + 1}</td><td className="l"><b>{x.manager}</b>{active.has(x.manager) && <span className="badge j">aktiv</span>}</td>
          <td className="l">{x.titles ? <span title={x.titleSeasons.join(', ')}>{strip(x.titles, '🏆')}</span> : <span className="muted">–</span>}</td><td>{x.second || ''}</td><td>{x.third || ''}</td><td><b>{x.podium}</b></td><td>{x.seasons}</td><td>{x.avgRank.toFixed(1)}</td><td>{Math.min(...x.ranks.map(r => r[1]))}.</td><td>{x.points}</td><td className="l mini">{x.first}{x.latest !== x.first ? ` – ${x.latest}` : ''}</td></tr>)}</tbody></table></div>
      <p className="mini">Punkte nach Formel-1-Wertung je Endrang (25-18-15-12-10-8-6-4-2-1). Bester = bester Endrang. Grau: nicht mehr dabei.</p></div>}
    <div className="grid2">

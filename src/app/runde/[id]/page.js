...
export default async function Runde({ params }) {
  ...
  let awards = []; if (passed) { const sd = await D.season(round.number); awards = roundAwards(sd, sd.upto.findIndex(r => r.id === round.id)); }
  ...
  return (<>
    ...
    {passed && <div className="card"><h3>Auszeichnungen des Spieltags</h3><Auszeichnungen awards={awards} me={u.manager_id} /></div>}
    <div className="grid2">

/* RLB-Emblem als Inline-SVG (Piktogramm-Spieler auf Rot, Wortmarke darunter). Klassen dienen der Intro-Animation.
   PlayerFigure ist die Spielerfigur allein (1000er-Raster), optional mit Namen und Nummer auf dem Rücken. */
export function PlayerFigure({ name, number, legClass = 'lg-leg', className = 'lg-player' }) {
  return (
    <g className={className} fill="#fff" stroke="#fff" strokeLinecap="round" strokeLinejoin="round">
      ...
      {name && <g className="lg-name" stroke="none" fill="#D20515" fontFamily="'Barlow Condensed','Arial Narrow',sans-serif" fontWeight="700" textAnchor="middle">
        <text transform="translate(358 318) rotate(14)" fontSize="44" letterSpacing="2">{String(name).toUpperCase()}</text>
        {number != null && <text transform="translate(346 392) rotate(14)" fontSize="72">{number}</text>}
      </g>}
    </g>
  );
}

export default function RlbLogo({ height = 40, wordmark = true, className = '', name, number }) {
  ...
  return (
    <svg className={`rlb-logo ${className}`} viewBox={`0 0 1000 ${vbH}`} height={height} width={height * 1000 / vbH} role="img" aria-label="RLB">
      ...
      <PlayerFigure name={name} number={number} />
